...
function cumlelereDonustur(cumleler, seperator=","){
	let birlesikCumleler = cumleler.map(current => current.join(seperator));
...
function paragrafOlustur(cumleler, cumleKur, cumlelereDonustur){
	let myArray = cumlelereDonustur(cumleler, " ");
	return cumleKur(myArray[1], myArray[3], myArray[5], myArray[7], myArray[9]);
}
...
function emojileriDonustur(argMessage, emojiler){
	for (let key in emojiler) {
		argMessage = argMessage.replaceAll(key.toUpperCase(), emojiler[key]);
		argMessage = argMessage.replaceAll(key.toLowerCase(), emojiler[key]);	
	}
	return argMessage;
}
